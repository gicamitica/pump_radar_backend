...
Runs isolated from main backend. Writes to MongoDB collection 'observation_dusk'.
...
DUSK_ADDR = "0x940a2dB1B7008B6C776d4faaCa729d6d4A4AA551"
...
def get_price_gecko():
    try:
        r = requests.get(
            f"https://api.geckoterminal.com/api/v2/networks/eth/tokens/{DUSK_ADDR}",
            timeout=15,
        )
        d = r.json()
...
def main():
    ...
    db = client[DB_NAME]
    col = db.observation_dusk

    now_ts = time.time()
    now_iso = datetime.now(timezone.utc).isoformat()

    price = get_price_gecko()
    whale_data = _safe_get(f"{API_BASE}/api/crypto/whale-movements/eth/{DUSK_ADDR}")
    market_data = _safe_get(f"{API_BASE}/api/crypto/dune/market-signals/eth/{DUSK_ADDR}")
