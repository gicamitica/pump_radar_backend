...
Runs isolated from main backend. Writes to MongoDB collection 'observation_vet'.
...
VET_ADDR = "0x6FDcdfef7c496407cCb0cEC90f9C5Aaa1Cc8D888"
...
def get_price_gecko():
    try:
        r = requests.get(
            f"https://api.geckoterminal.com/api/v2/networks/bsc/tokens/{VET_ADDR}",
            timeout=15,
        )
        d = r.json()
...
def main():
    ...
    db = client[DB_NAME]
    col = db.observation_vet

    now_ts = time.time()
    now_iso = datetime.now(timezone.utc).isoformat()

    price = get_price_gecko()
    whale_data = _safe_get(f"{API_BASE}/api/crypto/whale-movements/bsc/{VET_ADDR}")
    market_data = _safe_get(f"{API_BASE}/api/crypto/dune/market-signals/bsc/{VET_ADDR}")
