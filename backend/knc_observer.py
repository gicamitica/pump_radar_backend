...
Runs isolated from main backend. Writes to MongoDB collection 'observation_knc'.
...
KNC_ADDR = "0xe4DDDfe67E7164b0FE14E218d80dC4C08eDC01cB"
...
def get_price_gecko():
    try:
        r = requests.get(
            f"https://api.geckoterminal.com/api/v2/networks/arbitrum/tokens/{KNC_ADDR}",
            timeout=15,
        )
        d = r.json()
...
def main():
    ...
    db = client[DB_NAME]
    col = db.observation_knc

    now_ts = time.time()
    now_iso = datetime.now(timezone.utc).isoformat()

    price = get_price_gecko()
    whale_data = _safe_get(f"{API_BASE}/api/crypto/whale-movements/arbitrum/{KNC_ADDR}")
    market_data = _safe_get(f"{API_BASE}/api/crypto/dune/market-signals/arbitrum/{KNC_ADDR}")
