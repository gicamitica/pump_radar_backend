...
Runs isolated from main backend. Writes to MongoDB collection 'observation_bonk'.
...
def get_price_gecko():
    try:
        r = requests.get(
            f"https://api.geckoterminal.com/api/v2/networks/solana/tokens/{BONK_ADDR}",
            timeout=15,
        )
        d = r.json()
...
def main():
    ...
    db = client[DB_NAME]
    col = db.observation_bonk

    now_ts = time.time()
    now_iso = datetime.now(timezone.utc).isoformat()

    price = get_price_gecko()
    whale_data = _safe_get(f"{API_BASE}/api/crypto/whale-movements/solana/{BONK_ADDR}")
    market_data = _safe_get(f"{API_BASE}/api/crypto/dune/market-signals/solana/{BONK_ADDR}")
